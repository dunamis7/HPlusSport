...
namespace HPSportsPlus.Models.Advanced_Retrieval_Classes
{
    public static class SearchQuery
    {
        ...
        public static IQueryable<Product> CustomSearch( IQueryable<Product> products, ProductQueryParameters productQuery)
        {
            ...
            //if ((!string.IsNullOrEmpty(productQuery.Sku)) && (!string.IsNullOrEmpty(productQuery.Name)))
            //{
            //    products = products.Where(
            //    p => p.Name.ToLower().Contains(
            //    productQuery.Name.ToLower()) &&
            //            p.Sku.ToLower().Contains(productQuery.Sku.ToLower())
            //        );
            //}
